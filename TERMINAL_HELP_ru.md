...
Язык: [English](TERMINAL_HELP.md) · **Русский**

Основные команды для работы с проектом в терминале. Подставьте **свой** путь к клону репозитория вместо `/path/to/vpn-server-manager`. Домашние каталоги конкретных людей в публичный репозиторий не пишем.
...
```bash
source venv/bin/activate && python3 -m py_compile run.py
```
...
```bash
# VPN Server Manager
alias vpn-cd='cd /path/to/vpn-server-manager'
alias vpn-run='cd /path/to/vpn-server-manager && source venv/bin/activate && python3 run.py'
alias vpn-build='cd /path/to/vpn-server-manager && source venv/bin/activate && python3 build_macos.py'
alias vpn-key='cd /path/to/vpn-server-manager && cat .env'
```
...
- **[SECRET_KEY_ru.md](docs/SECRET_KEY_ru.md)** — шифрование
- **[BUILD_ru.md](BUILD_ru.md)** — сборка
- **[INDEX_ru.md](docs/INDEX_ru.md)** — индекс документации 