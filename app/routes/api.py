...
@api_bp.route('/monitoring/<server_id>/webpanels', methods=['GET'])
@require_auth
@require_pin
def get_webpanels(server_id):
    """Веб-панели стека (Dockhand, Headplane): детект + SSH-туннель + URL"""
    if not rate_limiter.is_allowed(f"server_{server_id}"):
        return jsonify({
            'success': False,
            'error': 'Rate limit exceeded. Please wait a moment.'
    ...
    try:
        ...
        data = ssh_service.get_webpanels_status(
            ip=creds['ip'],
            user=creds['user'],
            password=creds['password'],
            port=creds['port'],
        ...
        logger.error(f"Error getting webpanels for server {server_id}: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
